-- Migration 20260818001032 (0061_apply_registry_to_counterparties) — exportada de supabase_migrations.schema_migrations
-- Aplica o cadastro à carteira: preenche nome, regime e credit_transfer_pct das
-- contrapartes a partir do cache global. NUNCA sobrescreve regime definido
-- manualmente pelo usuário — a correção humana vence a fonte automática.
create or replace function apply_registry_to_counterparties(p_tenant uuid)
...
grant execute on function apply_registry_to_counterparties(uuid) to authenticated;

-- Quais CNPJs da carteira faltam no cache (ou estão velhos)? O serviço busca só esses.
create or replace function counterparties_missing_registry(p_tenant uuid, p_ttl_days int default 30)
returns table (cnpj text) language plpgsql stable security definer set search_path = public, extensions as $$
begin
...
grant execute on function counterparties_missing_registry(uuid,int) to authenticated;

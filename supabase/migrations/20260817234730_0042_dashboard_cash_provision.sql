-- Migration 20260817234730 (0042_dashboard_cash_provision) — exportada de supabase_migrations.schema_migrations
-- O card "Provisão sugerida" mostrava R$ 0,00 porque dashboard_cash não devolvia
-- a provisão no payload. Passa a devolver a provisão do mês corrente e a acumulada
-- no horizonte. Nada mais muda no contrato.
...
grant execute on function dashboard_cash(uuid,int) to authenticated;

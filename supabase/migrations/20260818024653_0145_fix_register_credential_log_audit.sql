-- Migration 20260818024653 (0145_fix_register_credential_log_audit) — exportada de supabase_migrations.schema_migrations
-- BUG BLOQUEANTE: register_credential chamava log_audit passando v_id (uuid) no
-- parâmetro p_entity_id, que é TEXT. O Postgres não resolve a sobrecarga e o
-- upload de certificado falhava com "function log_audit(...) does not exist" —
-- depois de o arquivo já ter sido cifrado e gravado no bucket.
--
-- Duas correções: o cast que faltava, e a limpeza do órfão. Se a auditoria falha
-- DEPOIS do insert, a credencial fica registrada mas o registro de auditoria não
-- existe — num sistema onde a auditoria é a prova, isso não pode passar em silêncio.
...
begin
  ...
  perform log_audit(p_tenant, 'credential.register', 'integration_credentials',
                    v_id::text,                      -- <- o cast que faltava
                    null,

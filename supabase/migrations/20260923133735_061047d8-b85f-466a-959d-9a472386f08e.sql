-- 0230: tabelas de referência (matriz CST × cClassTrib, versões de regra, cache de
-- cálculo) tinham política SELECT com USING (true): qualquer sessão autenticada,
-- inclusive uma sem vínculo nenhum com a plataforma, lia a base inteira.
-- Passa a exigir vínculo real (membership ativa) — nenhum usuário do produto perde
-- acesso, porque todo usuário do app pertence a pelo menos um tenant.

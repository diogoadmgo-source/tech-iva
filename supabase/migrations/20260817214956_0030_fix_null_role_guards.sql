-- Migration 20260817214956 (0030_fix_null_role_guards) — exportada de supabase_migrations.schema_migrations
-- FALHA DE SEGURANÇA REAL: guardas escritas como "if role_in(x) not in (...) then raise"
-- não disparam quando role_in() devolve NULL (usuário autenticado sem nenhuma membership
-- no tenant alvo). NULL NOT IN (...) = NULL, e IF NULL não executa o RAISE.
-- Consequência: qualquer usuário logado podia chamar channel_portfolio(qualquer_tenant)
-- e ver empresas, CNPJs e buracos de caixa. Corrigido com coalesce(... , false).
-- Helper para padronizar daqui em diante.
create or replace function has_role(p_tenant uuid, p_roles member_role[]) returns boolean
...
grant execute on function channel_portfolio(uuid,jsonb), set_regime_manual(uuid,uuid,regime_kind,text) to authenticated;

-- validação descartável do seed 0019 (cria tenant temporário e roda o seed nele)
do $$
declare v_platform uuid;
begin
  select id into v_platform from tenants where kind='platform' limit 1;
  if not exists (select 1 from tenants where name='ZZ Seed Validacao TMP') then
    insert into tenants (parent_id, kind, name, slug, cnpj, path, level)
    values (v_platform, 'company', 'ZZ Seed Validacao TMP', 'zz-seed-tmp', '00000000000199', 'tmp'::extensions.ltree, 0);
  end if;
end $$;
...
begin
  ...
  if v_tenant is null then
    raise exception 'tenant % nao encontrado', v_tenant_name;
  end if;

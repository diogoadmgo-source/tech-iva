-- 0017_commissions.sql — Documento 03 bloco 3.6 (/commissions do canal)
-- Cria:
--   commission_rules            regra de comissão do canal (pct sobre MRR + pct sobre crédito)
--   channel_commission_statement(p_tenant, p_month) -> jsonb   extrato mensal por empresa
--   set_commission_rule(p_tenant, p_mrr_pct, p_credit_pct, p_note) -> uuid
-- RLS: leitura por in_scope, escrita apenas plataforma (a regra é comercial da operadora).

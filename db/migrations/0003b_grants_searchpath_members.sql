-- 0003b_grants_searchpath_members.sql
-- Correções técnicas obrigatórias (decisões gravadas) + Auth/perfis (bloco 1.4) + gate de MFA.
--
-- 1) GRANTs: no Supabase o schema public NÃO concede privilégios por padrão a
--    anon/authenticated/service_role. Sem GRANT, RLS não basta: PostgREST devolve
--    "permission denied". Nenhum GRANT para anon (nada é público nesta fase).
-- 2) search_path: funções que usam pgcrypto (digest, gen_random_bytes) precisam de
--    `public, extensions` — aplicado nas definições de 0002/0004/0005.
-- 3) v_tenant_members → função tenant_members(p_tenant) validando in_scope(p_tenant).

-- ---------------------------------------------------------------- 1) GRANTs
grant usage on schema public to authenticated, service_role;

grant select, insert, update          on public.tenants       to authenticated;  -- sem delete: arquivar por status
...
grant select                          on public.audit_log     to authenticated;  -- append-only
...
-- Reforço do append-only do audit_log (o revoke de 0001 roda antes destes grants)
revoke update, delete on public.audit_log from authenticated;
revoke all on public.audit_log from anon;

-- ------------------------------------------- 2) Auth: profiles automático (bloco 1.4)
...
-- --------------------------------------------------- 3) MFA obrigatório (decisão gravada)
-- Papéis platform_* e channel_admin exigem AAL2 nas RPCs sensíveis.
-- owner/finance/commercial/viewer não exigem MFA nas RPCs de gestão.
...
-- Levanta exatamente 'MFA required' quando o papel efetivo exige AAL2 e a sessão é AAL1.
...
-- ------------------------------- 4) Membros do tenant: função em vez de view

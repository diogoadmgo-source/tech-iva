-- 0010_data_plane.sql — Documento 02, bloco A: schema do plano de dados
-- Depende de: 0001..0006 (tenants, rule_versions, in_scope, role_in, can_admin, log_audit)
-- Regras: RLS em TODAS as tabelas; leitura por in_scope(tenant_id); escrita pelo front
-- apenas nas tabelas de decisão do usuário. GRANTs explícitos (PostgREST).
...
-- partições mensais: 12 meses para trás, 36 para frente (job cria as futuras)
do $$
declare d date := date_trunc('month', now() - interval '12 months')::date;
begin
  while d < date_trunc('month', now() + interval '36 months')::date loop
    execute format(
      'create table if not exists %I partition of tax_cash_events for values from (%L) to (%L)',
...
-- ---------- escrita pelo front (tabelas de decisão do usuário) ----------
...
-- Todo o resto é escrito por service role (serviços) ou por RPC security definer.

-- ---------- auditoria em tabelas de decisão ----------
...
-- ---------- leitura rápida (refresh por job) ----------
-- MVs não respeitam RLS: sem grant para authenticated/anon; acesso só via RPC security definer.

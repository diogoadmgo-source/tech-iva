-- 0121_project_cash_modalidade.sql — ESPELHO da migration aplicada no banco.
--
-- project_cash_sql passa a receber a modalidade e a mudar o RITMO da saída:
--  - apuracao: o imposto do mês inteiro sai de uma vez, dia 20 do mês seguinte (degrau)
--  - rad/split: goteja, na data do recebimento (semanalmente no run-rate)
-- Quando p_modalidade é null, usa tenant_modalidade(p_tenant) — padrão 'apuracao'.

create or replace function public.project_cash_sql(p_tenant uuid, p_horizon_days integer default 120,
                                                  p_modalidade modalidade_recolhimento default null::modalidade_recolhimento)
returns jsonb
...
revoke execute on function public.project_cash_sql(uuid, integer, modalidade_recolhimento) from anon, authenticated;
grant execute on function public.project_cash_sql(uuid, integer, modalidade_recolhimento) to service_role;

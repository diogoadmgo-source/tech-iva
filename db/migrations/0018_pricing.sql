-- 0018_pricing.sql — Documento 03 bloco 3.7 (T3 Preço)
-- Cria:
--   price_scenario_compute(p_scenario)                     -> int    (recalcula price_lines)
--   price_scenario_create(tenant, name, margem, ano, cp)   -> uuid   (cenário + job price_scenario)
--   price_scenario_detail(p_scenario)                      -> jsonb  (cenário + linhas + totais)
--   approve_price_scenario(p_scenario)                     -> void   (aprova e arquiva o anterior)
--   update_product_price(product, custo, preco_atual)      -> void   (edição inline auditada)
-- Todas security definer com checagem explícita de papel (in_scope/role_in/is_platform).
...
-- Fator de aproveitamento de crédito na entrada pelo cliente, por regime.
...
-- NOTA: o seed de demonstração (produtos/clientes da Distribuidora Beta) foi movido
-- para db/seeds/0018_seed_dev_pricing.sql — dados fictícios, dev/staging apenas.

...
| 11 | função do banco `apuracao_divergencia` | `coalesce(v_receita.debitos_cents, 0)` na diferença e no "divergente" | `rtc_apuracao.debitos_cents` (`bigint`, nulo, sem default) | Sim — **sempre**. Nenhuma rotina grava `debitos_cents`: nem `rtc_apuracao_ingest_json`, nem `rtc_apuracao_upsert`, nem o código do app (única função que cita a coluna é a própria `apuracao_divergencia`). Toda apuração `disponivel` mostraria "Débito apurado pela Receita R$ 0,00" (o `?? 0` da tela). O veredito dependia do nosso lado: com notas calculadas, divergência = nosso cálculo inteiro e "Calculamos mais do que a Receita apurou"; **sem nenhuma nota** (nosso lado também vira 0 pelo `coalesce(sum(...),0)`), diferença 0 e "Seu cálculo bate com a apuração da Receita" — pior, porque é uma confirmação falsa, e é exatamente o caso de produção hoje (0 notas). | **Mente** | **Corrigido** (commit `edb8dec`): migração 0233 (diferença e veredito nulos sem o débito da Receita) + `compararApuracao` (função pura com teste) que garante o mesmo na tela antes de a migração ser aplicada. Falta decidir **qual** número da Receita comparar — ver abaixo. |
...
| 13 | função do banco `rtc_apuracao_ingest_json` | `coalesce(p_json->grupo->'debitos','[]')`, `coalesce((d->>'valorCBSTotal')::numeric,0)`, e no fim `status='disponivel'`, `resultado_cents` = soma ou 0, natureza `neutro` | O arquivo JSON que a Receita devolve (`rtc_apuracao.payload`) | Sim, se o formato real for diferente do manual. A função foi escrita a partir do manual e nunca rodou sobre um arquivo real (0 apurações `disponivel`, `rtc_debito` vazio em 25/09). Grupo com outro nome, `debitos` que não é lista ou débito sem `valorCBSTotal` viravam "nenhum débito" / R$ 0,00, e a apuração aparecia **disponível** com resultado zero e natureza neutra. Definição em produção conferida com `pg_get_functiondef` em 25/09 (igual à da migração 0156); permissões `postgres` (dono) e `service_role`. | **Mente** | **Corrigido nesta rodada** pela migração 0234 (`supabase/migrations/20260923200000_0234_ingestao_nao_entendeu_e_erro.sql`): arquivo que não se entende para com erro explícito antes de gravar; o arquivo bruto fica guardado para reprocessar. Lógica conferida com amostras em consulta só de leitura (ver relatório da rodada). Ainda não aplicada. |
| — | `src/lib/error-page.ts` | — | — | É CSS dentro de uma string (não é dinheiro). | Falso positivo | Nada. |

Resumo: 6 inofensivos (1, 3, 5, 6, 7, 8), 3 mentiras corrigidas (2, 11, 13), 3 mentiras
...
- `weekly_digest_batch` (e-mail semanal) — mesmo padrão dos itens 9 e 10:
  `gap_30_cents` e `gap_90_cents` saem de `coalesce(sum(...), 0)` sobre
  `tax_cash_events` (`supabase/migrations/20260817214256_2d860888-e18e-4f9b-bc5b-8fb4786fd095.sql`,
  linhas ~69-76; definição em produção conferida em 25/09, igual ao arquivo).
  Empresa nunca projetada recebe no e-mail "buraco de R$ 0,00", igual a uma
  empresa saudável. `tax_out_month_cents` (linhas ~77-79) tem o mesmo problema.
  Classificação: **Mente — precisa decisão** (a mesma decisão dos itens 9 e 10).

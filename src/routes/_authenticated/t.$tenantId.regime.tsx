...
function RegimeScreen() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <header>
        <h1 className="text-2xl font-semibold tracking-tight">Regime tributário</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Simples tradicional × híbrido de 2027 a 2033, com a carteira real da empresa.
        </p>
      </header>
      ...
      {/* -------------------------------------------------- passo 1 */}
      {step === 0 && (
        <section className="space-y-4 rounded-xl border border-border bg-surface-1 p-5 shadow-e1">
          <div className="flex items-baseline justify-between gap-4">
            <h2 className="text-sm font-semibold">Confirmar carteira (últimos 12 meses)</h2>
            <Link
              to="/t/$tenantId/chain"
              params={{ tenantId }}
            ...
            <>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {(
                  ...
                  <div key={title} className="rounded-lg border border-border bg-surface-2 p-3">
              ...
              {(w?.unknown_regime_count ?? 0) > 0 && (
                <p className="rounded-lg border border-flow-out/40 bg-flow-out/10 p-3 text-xs text-foreground">
                  {w?.unknown_regime_count} contraparte(s) com regime desconhecido — o resultado sai
                  com confiança parcial até classificá-las.
                </p>
              )}
            </>
          )}
      ...
      {step === 1 && (
        <section className="space-y-4 rounded-xl border border-border bg-surface-1 p-5 shadow-e1">
          <h2 className="text-sm font-semibold">Premissas</h2>
          ...
          <div className="flex items-center justify-between gap-4 rounded-lg border border-border bg-surface-2 p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Recupera crédito de entrada hoje perdido ({formatPct(w?.simples_supplier_share_pct ?? 0)} das compras).
              </p>
            </div>
            <Switch
              id="swap"
      ...
      {step === 2 && (
        <section className="space-y-4 rounded-xl border border-border bg-surface-1 p-5 shadow-e1">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-sm font-semibold">Rodar simulação</h2>
      ...
      {history.isLoading ? (
        ...
        <section className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-surface-1 p-4 shadow-e1">
            <h3 className="text-sm font-semibold">Carga por ano (2027–2033)</h3>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <div
              className={cn(
                ...
                countdown !== null && countdown <= 60
                  ? "border-flow-out/50 bg-flow-out/10"
                  : "border-border bg-surface-1",
              )}
            >
              <h3 className="text-sm font-semibold">Próxima janela de opção</h3>
            ...
            <div className="rounded-xl border border-border bg-surface-1 p-4 shadow-e1 lg:col-span-2">
                  ...
                  <li key={sim.id} className="flex flex-wrap items-center justify-between gap-3 py-2">
                    <div className="min-w-0">
                      <p className="text-sm">
                        ...
                        <span className="font-mono tabular">{formatPct(sim.results.delta_pct)}</span>{" "}
                        a favor do híbrido
                      </p>
